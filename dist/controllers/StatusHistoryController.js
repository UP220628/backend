...
exports.exportLogsToExcel = exports.getLogs = void 0;
const StatusHistoryService_1 = require("../services/StatusHistoryService");
const exceljs_1 = __importDefault(require("exceljs"));
const asyncHandler_1 = require("../utils/asyncHandler");
const constants_1 = require("../constants");
/** Build filters from query params + user context (shared between getLogs and export) */
function buildLogFilters(req, user) {
    const filters = {
        ...
        sort: (req.query.sort === 'alpha' ? 'alpha' : 'date'),
    ...
    if (user && user.roleId === constants_1.ROLE_IDS.CARRIER && user.providerId) {
...
const formatDate = (dateString) => {
    if (!dateString)
        return '';
    return new Date(dateString).toLocaleString('es-MX', { timeZone: constants_1.TIMEZONE });
};
...
exports.getLogs = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    const data = await StatusHistoryService_1.statusHistoryService.search(filters);
    res.json({ ok: true, data });
});
exports.exportLogsToExcel = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const user = res.locals.user;
    const filters = buildLogFilters(req, user);
    const data = await StatusHistoryService_1.statusHistoryService.search(filters);
    // Agrupar datos por unidad
    const groupedUnits = data.reduce((acc, row) => {
        if (!acc[row.unitId]) {
            acc[row.unitId] = {
                unitId: row.unitId,
                vin: row.vin,
    ...
    workbook.creator = constants_1.EXCEL_STYLES.APP_NAME;
    ...
    // ─── Estilo de cabecera ────────────────────────────────────────────────
    const thinBorder = { style: 'thin', color: { argb: constants_1.EXCEL_STYLES.BORDER_COLOR } };
    const cellBorders = {
    ...
    headerRow.eachCell(cell => {
        cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: constants_1.EXCEL_STYLES.NISSAN_RED } };
        cell.font = { bold: true, color: { argb: constants_1.EXCEL_STYLES.WHITE }, size: 11, name: constants_1.EXCEL_STYLES.FONT_NAME };
    ...
    units.forEach((unit, idx) => {
        const notesText = unit.notes.length > 0
            ? unit.notes.map((n) => {
                ...
                return `[${statusLabel}] ${d.toLocaleString('es-MX', { timeZone: constants_1.TIMEZONE })}: ${n.note}`;
            }).join('\n')
            : '';
        ...
        const rowBg = isEven ? constants_1.EXCEL_STYLES.ROW_NORMAL : constants_1.EXCEL_STYLES.ROW_ALT;
        row.eachCell({ includeEmpty: true }, cell => {
            cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: rowBg } };
            cell.alignment = { vertical: 'top', wrapText: true };
            cell.border = cellBorders;
            cell.font = { name: constants_1.EXCEL_STYLES.FONT_NAME, size: 10 };
        });
...
//# sourceMappingURL=StatusHistoryController.js.map